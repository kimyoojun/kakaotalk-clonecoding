<script>
  ...
  const friendAdd = async (event) => {
    if (event.key === "Enter") {
      ...
      const friendselect = await axios.post("/user/select", {"name": searchFriend})
  ...
  const friendAddBtn = async () => {
    console.log("추가")
    const friendadd = await axios.post("/user/add", {"my_name": localStorage.getItem('username'), "user_name": userIform})
    location.reload()
  }
